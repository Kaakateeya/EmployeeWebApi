...
namespace WebapiApplication.Api
{
    ...
    {
        private readonly IDashboardRequest IDashboardRequest; public DashboardRequestController() : base() { this.IDashboardRequest = new ImpDashboard(); }
        [HttpGet]
        ...
        public List<PartnerProfilesLatest> ExpressInterestSelectrequest([FromBody]DashboardRequest Dashboard) { return this.IDashboardRequest.ExpressInterestSelectBal(Dashboard); }
        public List<DashboardRequestChats> getCustometExpressIntrestDashBoardchats([FromUri]long? CustID, [FromUri]int? Status, [FromUri]int iStartIndex, [FromUri]int iEndIndex) { return this.IDashboardRequest.CustometExpressIntrestDashBoardchats(CustID, Status, iStartIndex, iEndIndex); }
        public List<TicketHistoryinfoResponse> GetTicketinformation([FromUri]long? Ticketid, [FromUri]char Type) { return this.IDashboardRequest.GetTicketinformationDal(Ticketid, Type); }
        public List<CommunicationHistry> DashboardCustometMessagesCount(CommunicationHistoryReq Mobj) { return this.IDashboardRequest.GetCustometMessagesCount(Mobj); }
